...
// Thêm biến để lưu log messages
const int MAX_LOG_MESSAGES = 10;  // Số lượng tin nhắn tối đa để hiển thị
...
// Hàm để thêm message vào log
void addLogMessage(String message) {
    if (logIndex >= MAX_LOG_MESSAGES) {
        // Quay về đầu buffer, ghi đè tin nhắn cũ
        logIndex = 0;
    }
...
void handleRoot() {
    ...
    html += "<div style='margin-top: 20px; padding: 10px; background-color: #4ee625;'>";
    html += "<h1>Data Monitoring</h1>";
    html += "</div>";
    html += "<p>Voltage (GPIO34): <span id='voltage'>--</span> V</p>";
    html += "<p>Current (GPIO36): <span id='current'>--</span> A</p>";
    html += "<p>Temperature (GPIO32): <span id='temperature'>--</span> &#8451;</p>";
    html += "<p>Cycle: <span id='cycle'>--</span></p>";
    html += "<p>Volt Current sensor calib: <span id='voltCurrentSensorCalib'>--</span> V</p>";
    html += "<p>Relay1 (GPIO25) State: <span id='relay1'>--</span></p>";
    html += "<p>Relay2 (GPIO26) State: <span id='relay2'>--</span></p>";
    html += "<p>Button (GPIO27) State: <span id='button'>--</span></p>";
    html += "<button onclick='toggleButton()'>Toggle Button</button>";
    // Thêm phần hiển thị log
    html += "<div style='margin-top: 20px; padding: 10px; background-color: #f0f0f0;'>";
    html += "<h3>System Messages</h3>";
    html += "<div id='logMessages' style='font-family: monospace;'></div>";
    html += "</div>";
    html += "<script>";
    ...
    html += "      document.getElementById('logMessages').innerHTML = data.logs.join('<br>');";
    html += "    });";
    html += "}";
    html += "updateData();";
...
void handleData() {
    ...
    // Thêm log messages vào JSON response
    json += "\"logs\":[";
